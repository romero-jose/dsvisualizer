...
def test_serialization_and_deserialization():
    operations = Operations(
        operations=[
            Operation(
                Init(0, 10, None), Metadata(animate=False, source="n1 = Node(10)")
            ),
            Operation(
                Init(1, 11, 0), Metadata(animate=False, source="n2 = Node(11, n1)")
            ),
            Operation(
                Init(2, 12, 1), Metadata(animate=False, source="n3 = Node(12, n2)")
            ),
...
def test_serialization_of_custom_classes():
    class MyCustomClass:
        def __init__(self):
            self.field = "test_value"

        def __str__(self):
            return f"MyCustomClass({self.field})"
    ...
    operations = Operations(
        operations=[
            Operation(Init(0, str(c1), None), Metadata(animate=True, source="")),
            Operation(SetValue(0, str(c2)), Metadata(animate=True, source="")),
        ]
    )
